...
async def post_command(client, message):
    try:
        await message.reply(
            "**Wᴇʟᴄᴏᴍᴇ Tᴏ Usᴇ Oᴜʀ Rᴀʀᴇ Mᴏᴠɪᴇ Pᴏsᴛ Fᴇᴀᴛᴜʀᴇ ᴡɪᴛʜ sʜᴏʀᴛᴇɴᴇʀ 🙂**\n\n"
    ...
    try:
        ...
        if chat_id in user_states:
            ...
            if current_state == "awaiting_num_files":
                ...
                if num_files_left > 0:
                    files_text = "file" if files_received == 1 else "files"
                    reply_message = await message.reply(f"⏩ Forward the No: {files_received + 1} {files_text}")
                    user_states[chat_id]["last_reply"] = reply_message
                else:
               ...
               # buttons = [
                 #   [InlineKeyboardButton(text=channel["name"], callback_data=f"post_{channel['chat_id']}") for channel in TARGET_CHANNELS]
               # ]
               # reply_markup = InlineKeyboardMarkup(buttons)
...
async def send_channel_selection(message):
    ...
    # Create a vertical button layout (one button per row)
    for chat_id, name in TARGET_CHANNELS.items():
        buttons.append([InlineKeyboardButton(text=name, callback_data=f"post_{chat_id}")])
